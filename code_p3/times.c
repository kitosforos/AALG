...
short average_search_time(pfunc_busqueda method, pfunc_key_generator generator, char order, int N, int n_times, PTIME_AA ptime)
{
  ...
  i = 0;
  if (!method || !generator || N < 0 || n_times < 0 || !ptime)
    return ERR;
  
  dict = init_dictionary(N, SORTED);
  if (!dict)
    return ERR;
  perm = generate_perm(N);
  if (!perm)
    return ERR;
  status = massive_insertion_dictionary(dict, perm, N);
  if (status == ERR)
    return ERR;
  keys = malloc(sizeof(int) * N * n_times);
  generator(keys, n_times * N, N);
  while (i < N*n_times)
  {
    start = clock();
    OBaux = method(dict->table, 0, N, keys[i++], &pos);
    end = clock();
    if (OBaux < OBlow || OBlow == 0)
      OBlow = OBaux;
    if (OBhigh < OBaux || OBhigh == 0)
